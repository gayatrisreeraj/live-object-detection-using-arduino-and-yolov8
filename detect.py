...
while True:
    ...
    while (time.time()-start_time)<=10:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if label in vehicle_classes or label=='person':
    ...
    while (time.time()-start_time)>10 and (time.time()-start_time)<=30:
        _, img = cap.read()
        cv2.imshow('YOLO V8 Detection', img)
        x = detected.intersection(vehicle_classes)
        if len(x)>0:
            arduino.sendData([1])  # Send '1' to Arduino
            print(f"Vehicle detected: {x}, entry allowed.")
        else:
